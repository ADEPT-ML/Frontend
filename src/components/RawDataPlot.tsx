...
function RawDataPlot(props: RawDataProps) {
    ...
    const lineWidth = 2;
    const lineColor = "rgba(150,150,150,1)";
    const areaColor = "rgba(150,150,150,0.5)";

    if (dateRangeDefined) {
        let shapes = []
        if (props.dateRange.start! > props.dateRange.min!) {
            shapes = shapes.concat([
                {
                    type: "rect",
                    xref: "x",
                    x0: props.dateRange.min,
                    x1: props.dateRange.start,
                    yref: "paper",
                    y0: 0,
                    y1: 1,
                    line: {
        ...
        if (props.dateRange.end! < props.dateRange.max!) {
            shapes = shapes.concat([
                {
                    type: "rect",
                    xref: "x",
                    x0: props.dateRange.end,
                    x1: props.dateRange.max,
                    yref: "paper",
                    y0: 0,
                    y1: 1,
                    line: {
